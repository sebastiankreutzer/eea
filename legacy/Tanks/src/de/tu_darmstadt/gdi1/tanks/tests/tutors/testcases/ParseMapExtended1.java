...
public class ParseMapExtended1 {
	...
	String stringRepresentation = "Map \"/assets/sandTexture.jpg\" \"correcte1\" \"null\" 0 0 0\n" +
							   	  ...
							   	  "Mine 60 20 303 168\n" +
							   	  "Mine 40 30 112 30\n";
	String stringRepresentation2 = "Map \"/assets/sandTexture.jpg\" \"correct01\" \"null\" 0 0 0\n" +
								   "Tank \"PlayerOne\" 1000 1000 10 10 3 3 30 5 0 10 300 200\n" +
	...
	@Test
	public final void testPlantMine() {
		adapter.initializeGame();
		adapter.loadMapFromFile(new File(map2));
		adapter.handleKeyPressN();
		assertEquals("Your map should not change when starting a new game", stringRepresentation2, adapter.getStringRepresentationOfMap());
		assertEquals("Mine entitiy count should be 0", 0, adapter.getMineCount());
		assertEquals("Playertank should have a maximum of 3 mines", 3, adapter.getMineMaxAmmo());
		assertEquals("Playertank should have 3 mines left", 3, adapter.getMineActualAmmo());
		adapter.handleKeyPressM();
		assertEquals("Mine entitiy count should be 1", 1, adapter.getMineCount());
		assertEquals("Playertank should have one mine ammo less", 2,  adapter.getMineActualAmmo());
		/*
		adapter.handleKeyPressM();
		assertEquals("Mine entitiy count should be 2", 2, adapter.getMineCount());
		assertEquals("Playertank should have 1 mine left", 1, adapter.getMineActualAmmo());
		adapter.handleKeyPressM();
		assertEquals("Mine entitiy count should be 3", 3, adapter.getMineCount());
		assertEquals("Playertank should have 0 mines left", 0, adapter.getMineActualAmmo());
		adapter.handleKeyPressM();
		assertEquals("Mine entitiy count should be 3", 3, adapter.getMineCount());
		assertEquals("Playertank should have 0 mines left", 0, adapter.getMineActualAmmo());
		*/
