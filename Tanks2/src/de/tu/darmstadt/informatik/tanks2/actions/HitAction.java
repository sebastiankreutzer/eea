...
public class HitAction extends EEAAction {
	...
	// colider.getOwnerEntity());
	//
	// Entity explosion = new Entity("Explosion");
	// explosion =new
	// ExplosionFactory(event.getOwnerEntity().getPosition().x-entity.getSize().y/2,
	// event.getOwnerEntity().getPosition().y-entity.getSize().y/2,
	// 0.01f, entity.getSize().y, entity.getSize().y,
	// Tanks.debug).createEntity();
	//
	// StateBasedEntityManager.getInstance().addEntity(sb.getCurrentStateID(),
	// explosion);
	// if(!life.hasLifeLeft()) {
	//
	// StateBasedEntityManager.getInstance().removeEntity(sb.getCurrentStateID(),
