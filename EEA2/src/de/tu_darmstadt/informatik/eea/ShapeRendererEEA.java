 ...
 * @author Nathan Sweet
 */
public class ShapeRendererEEA implements Disposable {
	...
	public ShapeRendererEEA() {
		this(5000);
	}

	public ShapeRendererEEA(int maxVertices) {
		this(maxVertices, null);
	}

	public ShapeRendererEEA(int maxVertices, ShaderProgram defaultShader) {
	 ...
	 * This can increase the number of batch flushes if care is not taken to
	 * draw the same type of shapes together. Default is false.
	 */
	 ...
	 *             if {@link #autoShapeType} is false.
	 */
	public void begin() {
		if (!autoShapeType)
	...
	/**
	 * Draws a line using {@link ShapeType#Line} or {@link ShapeType#Filled}.
	 * The line is drawn with two colors interpolated between the start and end
	 * points.
	 */
	 ...
	 * originX and originY specify the point about which to rotate the
	 * rectangle.
	 */
	...
	/**
	 * Draws a cube using {@link ShapeType#Line} or {@link ShapeType#Filled}.
	 * The x, y and z specify the bottom, left, front corner of the rectangle.
	 */
	...
	/**
	 * Calls {@link #arc(float, float, float, float, float, int)} by estimating
	 * the number of segments needed for a smooth arc.
	 */
	...
	/**
	 * Calls {@link #circle(float, float, float, int)} by estimating the number
	 * of segments needed for a smooth circle.
	 */
	...
	public void ellipse(float x, float y, float width, float height, float rotation) {
		int segments = Math.max(1, (int) (12 * (float) Math.cbrt(Math.max(width * 0.5f, height * 0.5f))));
		check(ShapeType.Line, ShapeType.Filled, segments * 3);
		float colorBits = color.toFloatBits();
		float angle = 2 * MathUtils.PI / segments;
		
		rotation = rotation * MathUtils.PI / 180;
		float sin = MathUtils.sin(rotation);
		float cos = MathUtils.cos(rotation);

		float cx = x + width / 2, cy = y + height / 2;
		float x1 = (width * 0.5f * MathUtils.cos(angle));
		float y1 = (height * 0.5f * MathUtils.sin(angle));
		if (shapeType == ShapeType.Line) {
	...
	/**
	 * Calls {@link #cone(float, float, float, float, float, int)} by estimating
	...
	/**
	 * Draws a polygon in the x/y plane using {@link ShapeType#Line}. The
	 * vertices must contain at least 3 points (6 floats x,y).
	 */
	...
	/**
	 * Draws a polyline in the x/y plane using {@link ShapeType#Line}. The
	 * vertices must contain at least 2 points (4 floats x,y).
	 */
